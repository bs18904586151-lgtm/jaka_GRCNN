...
class RealsenseD435(object):
    ...
    def get_data(self):
        ...
        aligned       = self.align.process(frames)          # ← 对齐
        ...
        color_img = np.asarray(color_frame.get_data())      # (H, W, 3) BGR
        depth_img = np.asarray(depth_frame.get_data())      # (H, W)    uint16 mm
...
#             print("✅ RealSense D435 硬件流水线已启动")
#         except Exception as e:
#             print(f"❌ 无法启动 RealSense: {e}")
#             raise e # 抛出异常让 UR_Robot 知道初始化失败了

#         # 预设内参矩阵
#         self.intrinsics = np.array([607.879, 0, 325.14, 0, 607.348, 244.014, 0, 0, 1]).reshape(3,3)

#     def get_data(self):
#         # 2. 直接获取当前帧，不要再启动 pipeline 了
#         frames = self.pipeline.wait_for_frames()
...
#         self.intrinsics = np.fromstring(data[0:(9*4)], float).reshape(3, 3)
#         depth_scale = np.fromstring(data[(9*4):(10*4)], float)[0]
